...
public class JB_HealthSystem : MonoBehaviour
{   
    public int vida = 5;
    ...
    public JB_LifeUIController uiController;
    ...
    public void TakeDamage(int amount)
    {
        ...
        if(vida <= 0)
        {
            ...
            //Logica da morte aqui

            if(CompareTag("Player"))
            {
                //Logica especial para o jogador
                //Exemplo: desabilitar controle, tocar animação, recarregar cena, etc.
                Debug.Log("Player morreu! Aqui você pode chamar animação ou reiniciar a fase.");
                SceneManager.LoadScene(SceneManager.GetActiveScene().name);
            }
            else{
                // Se não for o jogador, destrói o objeto
                Destroy(gameObject);
            }
        }
        else
    ...
    {
        estaInvencivel = true;
        //Aqui voc~e pode adicionar um efeito visual, tipo piscar o sprite
