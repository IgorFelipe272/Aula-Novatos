...
public class JB_Hit : MonoBehaviour
{   
    public int dano = 1;
    ...
    public JB_Patrol patrol;

    void Start()
    {
        if(patrol == null)
        {
            Debug.Log("JB_Patrol não foi atribuido.");
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            //Aplica dano
            JB_HealthSystem healtySystem = collision.GetComponent<JB_HealthSystem>();
            if (healtySystem != null)
            {
                healtySystem.TakeDamage(dano);
            }
    ...
    private IEnumerator AplicarKnockbackComDelay(Rigidbody2D playerRb, Vector2 knockbackDir, float duracaoControleDesativado)
    {
        ...
        JB_Script mov = playerRb.GetComponent<JB_Script>();
